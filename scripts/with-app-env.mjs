...
/** Resolve a local node_modules CLI so Windows can find vite.cmd / vite.js. */
export function resolveSpawn(command, args, root = projectRoot()) {
  ...
  if (existsSync(localCmd)) {
    return { cmd: localCmd, args, shell: win };
  }
  return { cmd: command, args, shell: win };
}

function withLocalBinPath(root, env) {
  const bin = join(root, "node_modules", ".bin");
  const pathKey = process.platform === "win32" && env.Path && !env.PATH ? "Path" : "PATH";
  const current = env[pathKey] || env.PATH || env.Path || "";
  return { ...env, [pathKey]: `${bin}${delimiter}${current}` };
}
...
function main(argv) {
  ...
  const env = withLocalBinPath(root, mergeAppEnv(readAppEnv(root), process.env));
